...
###  2. 理解JS的浮点数
JS和其他语言不同，JS只有一种数值数据类型，那就是`number`,不管是整数还是浮点数，JS都将其归类为数字。
```js
typeof 16; //"number"
typeof 98.5; //"number"
typeof -2.1; //"number"
...
实例如:`8|1; //9`,其实是将8转化为`1000`,1转化为`1`(多余的前导0省略了),然后进行位运算,  

可以通过`(8).toString(2); // "1000"`查看数字类型的二进制,toString方法的参数表示进制   
通过`parseInt('1001',2); // 9` 可以将字符串转按照对应进制转化为十进制的数  


**使用浮点数的时候需要注意:**
由于精度有限,即使是64位,但还是存在误差,所以对小数的浮点数进行操作的时候最好将其转化为整数先(银行内就是这样做的,按照最小的货币单位进行计算,比如分),  
```js
...
### 3. JS中的隐式的强制转换
`3 + true; // 4` 就是由于JS对类型的隐式转换导致的,在大多数情况下,JS不会抛出错误,而是按照多种多样的自动转换方式将值强制转换为期望的类型,例如:  
...
一般的,字符串的强制转换远比数字的强制转换更常见,有用.最好避免使用`valueOf`,除非对象确实是一个数字的抽象,并且  
`obj.toString()`能产生一个`obj.valueOf()`的字符串表示.
...
typeof "Hello"; // "string"
typeof s; // "object"
// 这意为这,不能用内置的操作符比较两个截然不同的String对象的内容
...
其存在的主要用处为可以使用他们的实用方法, 结合隐式强制转换,可以方便的使用这些方法:  
当对原始值提取属性和进行方法调用时,它表现得想已经使用了对应对象类型封装了该值一样.
```js
"hello".toUpperCase(); // "HELLO"
// 而隐式封装的一个结果就是可以对原始值设置属性,但对其没有任何用
...
### 5. 避免对混合类型使用 == 运算符
`"1.0e0" == {valueOf: function(){return true;}};` 表达式的值是什么?  
结果为`true`, 由于隐式强制转换, 在比较之前,他们都会转为数字, 字符串解析为数字1,  
匿名对象调用自身的`valueOf`方法得到true, 然后再转换为数字, 得到1;

一般的,JS显示调用Number函数或者一些一元运算符+将值转换为数字,   
而更好的替代方式是使用严格相等运算符,当两个参数为同一类型时,   
`==` 和 `===`没有差别, 但最好使用 `===`
[==转换规则][1.png]
...
**1. 分号仅在 { 标记之前, 一个或多个换行之后 或 程序输入的结尾 被插入**   
